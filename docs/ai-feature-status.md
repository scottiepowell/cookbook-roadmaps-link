...
| Recipe-session regression baseline | Pass | 8 deterministic session eval cases in `run_evals.py` | Covers draft generation, clarification, method-change RAG refresh, chatter/formatting no-refresh, clarification answer, demo finalize, and missing-session safety with generated fixtures only. |
