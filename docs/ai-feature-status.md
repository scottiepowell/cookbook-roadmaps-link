...
| Git Bash public stack launcher | Implemented; host startup verification blocked | [Windows Local Development](windows-local-development.md#start-the-public-cookbook-stack-from-git-bash) | `scripts/start-public-cookbook.sh` starts Docker Desktop and the existing public stack with bounded safe checks. The current host must clear a stale Docker `dockerInference` runtime socket via an elevated WSL service restart or Windows reboot before end-to-end startup can be rerun. |
