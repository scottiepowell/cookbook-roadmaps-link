...
| Save-to-Cookbook core adapter path | Decision complete, implementation blocked | [Save-to-Cookbook Core Adapter Path Decision](save-to-cookbook-core-adapter-path-decision.md) | Recommends the 0033W follow-up to bootstrap a source-owned/forked core workspace/custom image, with a reviewed upstream plugin/API hook as an alternative. External-image simulation remains prototype-only; direct DB writes and browser/session automation are rejected. |
