...
| Free-tier LLM offload and QMD-assisted chat ADR | Complete, docs-only | [Free-Tier LLM Offload and QMD-Assisted Chat ADR](free-tier-llm-offload-qmd-chat-adr.md) | primary-source review; static repository validation | Recommends Groq-hosted `openai/gpt-oss-20b` for a future bounded advisory pilot, not final answers. QMD remains benchmark-only; runtime integration still requires a separate task and the existing fact/eval gates. |
