...
| Public Google OIDC callback | Corrected; manual authenticated return pending | [0034N Results](../outbox/0034N-public-homepage-login-and-google-oidc-callback-results.md) | Public auth now uses the exact HTTPS callback and only `openid email profile`; Google accepts the authorization request without redirect mismatch. Account selection and final core session acceptance require a manual operator retry. |
