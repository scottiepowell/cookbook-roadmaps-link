...
| Secondary provider fact register and implementation gate | Complete, docs-only | [AI Secondary Provider Fact Register](ai-secondary-provider-fact-register.md), [AI Secondary Provider Implementation Gate](ai-secondary-provider-implementation-gate.md), `evals/ai_cookbook/secondary_provider_fact_gate_eval.py` | docs-only tests, offline fact-gate eval | Keeps `GLM-4.7 Flash` blocked and unverified unless primary provider documentation is recorded, requires a separate future runtime adapter task, and preserves the corrected 0030 baseline including `0030P` no-bake cheesecake behavior. |
| Free-tier LLM offload and QMD-assisted chat ADR | Complete, docs-only | [Free-Tier LLM Offload and QMD-Assisted Chat ADR](free-tier-llm-offload-qmd-chat-adr.md) | primary-source review; static repository validation | Recommends Groq-hosted `openai/gpt-oss-20b` for a future bounded advisory pilot across RAG, conversation, recipe-assistance, planning, support/operations, localization, and offline-development tasks. Final authoritative outputs stay on baseline/core paths. QMD remains benchmark-only; runtime integration still requires a separate task and the existing fact/eval gates. |
| Cookbook AI plugin/adapter architecture | Proposed, docs-only | [Cookbook AI Plugin and Adapter Architecture ADR](cookbook-ai-plugin-adapter-architecture-adr.md) | docs/static validation | Defines a future core-app → Cookbook AI Adapter → RAG/AI sidecar boundary, manifest/API sketches, recipe read/write ownership, staged migration, contract tests, and seamless native UX requirements without implementing integration. |
| AI importer Save-to-Cookbook adapter design | Proposed, docs-only | [AI Importer Save-to-Cookbook Adapter Design](ai-importer-save-to-cookbook-adapter-design.md) | docs/static validation | Defines a core-app-owned candidate/dry-run/commit boundary, review/edit/confirm flow, unknown-schema discovery, duplicate/idempotency handling, backup/rollback gates, and disposable local fixture phases. No button, endpoint, database mutation, or production write-back exists. |
| AI importer Save-to-Cookbook fixture contract | Complete, fixture-only/local | `ai-api/app/cookbook_import_adapter.py` | `test_cookbook_import_adapter.py`, pytest | Maps validated importer drafts to an in-memory candidate, returns safe field errors, duplicate signals, schema/version metadata, and idempotent replay/conflict results. No route, database access, production target, or write-back exists; upstream write schema remains unknown. |
| AI importer Save-to-Cookbook dry-run operation | Complete, internal/local-only | `ai-api/app/cookbook_import_dry_run.py` | `test_cookbook_import_dry_run.py`, pytest | Provides a disabled-by-default service envelope around the fixture adapter. Explicitly enabled callers receive no-write mapping/validation results; no HTTP route, commit operation, database access, or production target exists. |
| Local Vanilla Cookbook schema discovery | Complete, read-only/local | [Local Vanilla Cookbook Schema Discovery](local-vanilla-cookbook-schema-discovery.md) | Docker start/check, read-only Prisma/SQLite metadata inspection | Found the local Recipe model, ownership/time requirements, text ingredient/direction/servings fields, category/photo relations, and authenticated native create/update/delete source paths. Phase 3 write/rollback remains blocked pending core mapping and restore harness. |
| Save-to-Cookbook schema-informed write plan | Complete, design-only | [Save-to-Cookbook Schema-Informed Write Plan](save-to-cookbook-schema-informed-write-plan.md) | docs/static validation | Defines conservative first-write scope, synthetic ownership, deterministic text mapping, backup/restore, cleanup, duplicate/idempotency, failure injection, and localhost-only harness guards. No harness or mutation is implemented; Phase 3 remains blocked. |
| QMD local hybrid retrieval | Candidate, docs-only | [QMD Local Hybrid Retrieval Adapter Spike](qmd-local-hybrid-retrieval-adapter-spike.md) | external high-level review; local static validation | QMD remains an optional local candidate behind a future `RetrievalBackend`/`RetrievalAdapter` boundary. No QMD dependency, Node runtime, model download, generated snapshot/index, vector/embedding implementation, route, or production integration is accepted. Current deterministic keyword retrieval remains the default. |
| Portfolio platform AWS scaling architecture | Proposed, docs-only | [Portfolio Platform AWS Scaling Architecture ADR](portfolio-platform-aws-scaling-architecture-adr.md) | docs/static validation | Chooses a staged EC2-first path: single Compose host, production-shaped EC2, ALB/ASG, then ECS on EC2, with Fargate/EKS only if justified. Defines app silos, shared infrastructure, portfolio metadata, AI budget/kill-switch controls, migration triggers, and operational evidence without creating resources or changing deployment. |
...
| Recipe-session eval harness | Complete | `evals/ai_cookbook/session_eval.py`, `evals/ai_cookbook/session_cases.yaml` | offline eval tests, `run_evals.py` | Regression-tests start/message/get/finalize session flows with generated fixtures and mock provider, including clarification, RAG refresh, no-refresh chatter/formatting, demo finalize, finalize-without-draft, equipment/exclusion refreshes, missing-session safety, leakage checks, and the no-bake cheesecake clarification regression path. |
...
| Local browser demo launch | Complete | `scripts/start-ai-demo-local.ps1` | pytest, mock demo | Seeds generated saved recipes and dataset fixtures and starts `/product`/`/demo` on `127.0.0.1:8000`. Automatically imports ignored `.env` without overwriting process values; valid local live config uses only `gpt-5.4-nano`, defaults to 500 output tokens, allows 500..1000, and preserves existing live/budget gates. `-Provider mock` preserves deterministic validation, and startup output redacts key presence. |
...
| Local Playwright UI troubleshooting | Complete, optional/local | `tests/ui/cookbook-ai-mode.spec.ts`, `scripts/run-ui-playwright.ps1` | Playwright Chromium harness | Checks product/demo navigation, selector propagation, normalized request payloads, safe live-unavailable UI, mock metadata, and desktop layout without live OpenAI. Artifacts are ignored. |
| Local mode acceptance audit | Complete, bounded/manual | mock smoke, Playwright mock suite, no-argument readiness check | local operator audit | Mock workflows report `mock/mock-basic`; Live selected on a mock server is controlled unavailable. The local profile was live-capable as `openai/gpt-5.4-nano`; one importer attempt returned HTTP 503 and was not retried. `scripts/diagnose-live-importer.ps1` provides approval-gated, one-call redacted diagnostics with model-field, port, and output-cap preflight guidance. |
...
| Live script env-file loader | Complete, local-only | `scripts/lib/ai-env-file.ps1`, `scripts/test-ai-env-file-loader.ps1`, launcher | PowerShell script test, pytest docs check | Loads ignored local `.env` files for explicit live wrappers and automatically for the local launcher, preserves existing process values, appends only safe missing defaults, and redacts secret-like names in summaries. |
...
| Local Cookbook AI product shell | Complete, local-only | `GET /product` links external Vanilla Cookbook and the AI sidecar workspace with safe readiness status |
| Local product acceptance baseline | Pass, offline | `/product` smoke, redirects, readiness safety, and Recipe Session Alpha checklist gate platform planning |
...
| Recipe-session regression baseline | Pass | 11 deterministic session eval cases in `run_evals.py`; 39 total offline eval cases | Covers draft generation, clarification, method/equipment/exclusion RAG refresh, chatter/formatting no-refresh, clarification answer, demo finalize, finalize-before-draft, missing-session safety, and the `make dessert -> cheesecake, no-bake, for 4 people` regression so no-bake method constraints stay preserved through retrieval query building and importer draft shaping. |
...
| Local live importer diagnostics | Pass, manual-only | offline sanitizer tests plus runbook diagnostic and dedicated importer smoke script | Explicit manual `openai` / `gpt-5.4-nano` acceptance passed at 500 and 1000 output tokens; 500 is the recommended manual cap. 400 failed safely with `output_cap_or_incomplete_response` / `JSONDecodeError`, and 300 is also too low for complete strict-schema JSON. 1000 remains a troubleshooting ceiling. Preflight plus explicit approval permits exactly one bounded call per invocation; normal validation remains mock/offline with no retries. |
...
| Save-to-Cookbook native local spike | Blocked, read-only review | `outbox/0033T-local-native-save-to-cookbook-spike-results.md` | Upstream create route requires Lucia-authenticated session state and has post-create image/embedding side effects without a safe adapter rollback envelope. Route was not called; native save remains unimplemented. |
| Save-to-Cookbook core adapter path | Local commit verified; local UI bridge complete | [Core-Owned Local Auth Commit Verification](core-owned-local-auth-commit-verification.md) | 0033Z/0034G provide the gated core commit evidence, and 0034H wires the importer review panel to the persistent local transport under explicit loopback/image/approval gates. Production save remains unimplemented. |
| Save-to-Cookbook sidecar real-save wiring | Planned/gated; not implemented | [Sidecar Real-Save Local Wiring Plan](sidecar-real-save-local-wiring-plan.md) | 0034B evaluates the core fixture bridge and rejects cookie/session export and direct sidecar DB writes. Next recommended step is a core-owned dev-only synthetic auth fixture; sidecar UI real-save wiring remains disabled. |
| Save-to-Cookbook core dev auth fixture | Complete, core-process/local-only | [Core-Owned Dev-Only Synthetic Auth Fixture](core-owned-dev-only-synthetic-auth-fixture.md) | 0034C verifies dry-run-before-commit, synthetic in-process ownership, real disposable persistence, replay/conflict, duplicate review, rollback, backup/restore, and safe output. Sidecar UI real-save wiring remains deferred; production save remains unimplemented. |
| Google-first core OIDC local auth spike | Complete, external core/local mock foundation | [Google-First Core OIDC Local Auth Spike](google-first-core-oidc-local-auth-spike.md) | 0034E adds identity-only Google descriptor/config guards, bounded profile normalization, core-owned provider-link mapping, and offline tests. No route, token, session, storage scope, provider call, or production auth. |
| Sidecar-to-core local real-save transport | Complete, disabled-by-default/local-only | [Sidecar-to-Core Local Real-Save Transport](sidecar-to-core-local-real-save-transport.md) | 0034F adds the guarded client; 0034H wires the review panel to the opt-in persistent `local/vanilla-cookbook-adapter:0034g` path. No persistent user session, production save, or provider call. |
...
| Manual local Google OIDC login verification | Complete, loopback/manual | [Manual Local Google OIDC Login Verification](manual-local-google-oidc-login-verification.md) | 0034M completed Google callback and core-owned Lucia session acceptance with exactly `openid email profile`; an authenticated settings page proved the `locals.user` authorization boundary. Replay/logout were not exercised in this run. Authenticated saved-recipe observation is the next separate task. |
| Local core Cloudflare tunnel | Running, Windows-hosted connector | [Local Core Cloudflare Tunnel](local-core-cloudflare-tunnel.md) | The existing `cookbook.roadmaps.link -> http://app:3000` remotely managed route required no dashboard/DNS change. A dedicated public core and connector are running on a private Docker network; public health returned HTTP 200. This does not enable production Google auth. |
...
| Git Bash public stack launcher | Complete and host-verified | [Windows Local Development](windows-local-development.md#start-the-public-cookbook-stack-from-git-bash) | `scripts/start-public-cookbook.sh` starts Docker Desktop and the existing public stack with bounded safe checks. After clearing a stale Docker `dockerInference` runtime socket, engine readiness, sidecar health, and local/public core health passed. |
