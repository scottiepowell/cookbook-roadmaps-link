...
| Hosted free-tier LLM offload ADR | Complete, docs-only | [Hosted Free-Tier LLM Offload ADR](free-tier-llm-offload-qmd-chat-adr.md) | primary-source review; static repository validation | Selects GroqCloud's hosted API with `openai/gpt-oss-20b` for a future bounded advisory pilot across RAG, conversation, recipe-assistance, planning, support/operations, localization, and offline-development tasks. Self-hosted/local inference and QMD are outside this solution. The ADR includes manual GroqCloud, local secret, and GitHub environment setup. Runtime integration still requires a separate task and existing fact/eval gates. |
