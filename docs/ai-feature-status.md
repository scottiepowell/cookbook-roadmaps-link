...
| Save-to-Cookbook core adapter path | Local commit verified; route/UI wiring blocked | [Core-Owned Local Auth Commit Verification](core-owned-local-auth-commit-verification.md) | 0033Z added the gated authenticated core commit route; 0034A verified one real service-level commit in a temporary SQLite DB with synthetic `AuthUser`, replay/conflict, duplicate blocking, rollback, and backup/restore. Route-level session verification and sidecar UI real-save wiring remain blocked by credential/session boundaries; production save remains unimplemented. |
...
| Sidecar-to-core local real-save transport | Complete, disabled-by-default/local-only | [Sidecar-to-Core Local Real-Save Transport](sidecar-to-core-local-real-save-transport.md) | 0034F adds a guarded sidecar client for the separate core synthetic verification route and the opt-in `local/vanilla-cookbook-adapter:0034f` image. No normal UI wiring, persistent user session, production save, or provider call. |
