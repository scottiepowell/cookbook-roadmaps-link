...
| Save-to-Cookbook core adapter path | Workspace bootstrapped; adapter blocked | [Source-Owned Vanilla Cookbook Workspace Bootstrap](source-owned-vanilla-cookbook-workspace-bootstrap.md) | Separate recursive checkout and local `local/vanilla-cookbook-adapter:0033w` image exist outside this repo. 0033X must add only the core-owned authenticated dry-run adapter; external-image simulation remains prototype-only. |
