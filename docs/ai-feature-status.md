...
| Manual local Google OIDC login verification | Blocked, core-local configuration and Docker unavailable | [Manual Local Google OIDC Login Verification](manual-local-google-oidc-login-verification.md) | 0034M safely found no ignored configuration in the external core workspace; a sidecar env file is not an approved core-local loopback configuration, and the local Docker engine is unavailable. No Google call or login was attempted. Authenticated saved-recipe observation remains a later, separately approved task. |
