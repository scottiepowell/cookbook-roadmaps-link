...
| Local live importer diagnostics | Pass, manual-only | offline sanitizer tests plus runbook diagnostic and dedicated importer smoke script | Uses a deterministic tiny fixture and defaults to 300 tokens; the recorded 300-token live diagnostic still failed with `output_cap_or_incomplete_response`. An explicit `-MaxOutputTokens 1000` profile permits one approved call per operator run only; normal validation remains mock/offline and no retries are used. If successful, operators dial down manually: 1000 -> 800 -> 600 -> 500 -> 400 -> 300. |
