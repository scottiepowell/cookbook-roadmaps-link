...
| Save-to-Cookbook core adapter path | Dry-run complete; commit blocked | [Core-Owned Local Dry-Run Adapter](core-owned-local-dry-run-adapter.md) | 0033X added the no-mutation service and local `local/vanilla-cookbook-adapter:0033x` image outside this repo. A separately reviewed authenticated commit boundary is still required; production save remains unimplemented. |
