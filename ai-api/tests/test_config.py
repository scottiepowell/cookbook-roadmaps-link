...
def test_ai_config_reports_provider_booleans(monkeypatch):
    ...
    monkeypatch.setenv("GROQ_API_KEY", "fake-groq-secret-value")
    ...
    assert "fake-groq-secret-value" not in response_text
...
def test_ai_config_defaults_to_unconfigured(monkeypatch):
    for name in ("OPENAI_API_KEY", "GROQ_API_KEY", "ANTHROPIC_API_KEY", "GOOGLE_API_KEY", "OLLAMA_BASE_URL"):
