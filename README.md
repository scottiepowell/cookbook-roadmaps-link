...
`0033G` adds the [Free-Tier LLM Offload and QMD-Assisted Chat ADR](docs/free-tier-llm-offload-qmd-chat-adr.md). It recommends Groq-hosted `openai/gpt-oss-20b` for a future bounded advisory pilot, keeps the OpenAI final-answer baseline, and requires measured call avoidance, token reduction, retrieval quality, citation fidelity, latency, privacy, and fallback evidence. QMD remains an optional local retrieval benchmark rather than an accepted dependency.
