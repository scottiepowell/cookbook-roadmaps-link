...
- [Public authenticated AI recipe importer](docs/public-authenticated-ai-importer.md)
...
- [Hosted free-tier LLM offload ADR](docs/free-tier-llm-offload-qmd-chat-adr.md)
...
`0033G` adds the [Hosted Free-Tier LLM Offload ADR](docs/free-tier-llm-offload-qmd-chat-adr.md). It selects GroqCloud's externally hosted API with `openai/gpt-oss-20b` for a future bounded advisory pilot across retrieval and other cheaply verifiable tasks such as intent classification, conversation compression, recipe metadata suggestions, shopping-list organization, support routing, and aggregate usage-report narratives. Self-hosted and local inference are outside the solution space. The ADR gives the exact adapter design plus manual GroqCloud, local `.env`, and GitHub environment setup steps. It keeps the OpenAI final-answer baseline and requires measured acceptance, correction effort, call avoidance, total-token reduction, quality, latency, privacy, and fallback evidence.
...
The local upstream schema was then inspected read-only in [Local Vanilla Cookbook Schema Discovery](docs/local-vanilla-cookbook-schema-discovery.md). The disposable app exposes a Prisma/SQLite `Recipe` model with UUID IDs, required owner/time fields, text ingredient/direction storage, text servings, relational categories, and separate photos. Its authenticated native create path was not called. The candidate mapping is not write-ready until serialization, category, ownership, and backup/rollback policies are approved.

The next readiness plan is documented in [Save-to-Cookbook Schema-Informed Write Plan](docs/save-to-cookbook-schema-informed-write-plan.md). It recommends a future first disposable test with one synthetic local owner and one recipe, deterministic text serialization, no categories/media/embeddings, strict localhost guards, backup/restore, and failure cleanup. It does not implement or authorize a write harness; Phase 3 remains blocked.
...
wiring has been added. 0034C now provides that core-process fixture with
dry-run-before-commit and disposable backup/restore; sidecar UI wiring remains
deferred.
...
0034N records the completed [Public Homepage Login Goal](docs/public-homepage-login-goal.md),
which makes login the primary returning-user action and moves initial
