...
- [Free-tier LLM offload and QMD-assisted chat ADR](docs/free-tier-llm-offload-qmd-chat-adr.md)
...
`0033G` adds the [Free-Tier LLM Offload and QMD-Assisted Chat ADR](docs/free-tier-llm-offload-qmd-chat-adr.md). It recommends Groq-hosted `openai/gpt-oss-20b` for a future bounded advisory pilot across retrieval and other cheaply verifiable tasks such as intent classification, conversation compression, recipe metadata suggestions, shopping-list organization, support routing, and aggregate usage-report narratives. It keeps the OpenAI final-answer baseline and requires measured acceptance, correction effort, call avoidance, total-token reduction, quality, latency, privacy, and fallback evidence. QMD remains an optional local retrieval benchmark rather than an accepted dependency.
